(() => {
 ...
 DKD.buy = id => {if(DKD.state.owned.includes(id))return false;DKD.state.owned.push(id);DKD.state.transactions.push({id,date:new Date().toISOString(),amount:DKD.game(id).price,type:'demo'});DKD.persist();DKD.download(id);return true;};
