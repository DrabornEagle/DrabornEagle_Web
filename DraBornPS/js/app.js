(() => {
 ...
 DKD.setWallpaper=id=>{if(wallpaper===id)return;wallpaper=id;wallpaperIndex=1-wallpaperIndex;const next=document.getElementById(wallpaperIndex?'wallpaper-b':'wallpaper-a'),previous=document.getElementById(wallpaperIndex?'wallpaper-a':'wallpaper-b');next.style.backgroundImage=id==='default'?"url('assets/wallpapers/dkd-miami-v06.webp')":`url('${DKD.art(id)}')`;next.classList.add('shown');previous.classList.remove('shown');};
 ...
 'friend':id=>{const friend=DKD.friends.find(item=>item.name===id);DKD.modal(friend.name,`<div class="row">${DKD.avatar(friend.name,true,friend.color)}<div><h3>${friend.name}</h3><p>Playing ${friend.game}</p></div></div><p class="small muted">Demo friend · This profile is part of your local console.</p><button class="pill primary" data-action="invite-friend" data-id="${id}">Invite to Party</button>`);},
 'invite-friend':id=>{DKD.closeModal();DKD.notify('Party created',id+' has joined your demo party.');},
 ...
 'transactions':()=>DKD.modal('Transaction History',DKD.state.transactions.length?DKD.state.transactions.map(transaction=>`<div class="setting-row"><span>${DKD.game(transaction.id).title}<small>${new Date(transaction.date).toLocaleDateString()} · Demo purchase</small></span><span>${DKD.price(transaction.amount)}</span></div>`).join(''):'<p>No demo purchases yet.</p>'),
 ...
 'trailer':()=>DKD.modal('Game Showcase',`<img src="${DKD.art('apex')}" alt="APEX DRIFT" style="width:100%;border-radius:12px"><p>APEX DRIFT · Five stages of midnight arcade racing.</p><button class="pill primary" data-action="launch" data-id="apex">Play the Demo</button>`),
