(() => {
 ...
 DKD.saveProfilePhoto=file=>new Promise((resolve,reject)=>{if(!file||!file.type?.startsWith('image/')){reject(new Error('Please choose an image file.'));return;}const reader=new FileReader();reader.onerror=()=>reject(new Error('The selected image could not be read.'));reader.onload=()=>{const image=new Image();image.onerror=()=>reject(new Error('The selected image could not be opened.'));image.onload=()=>{const size=320,canvas=document.createElement('canvas'),context=canvas.getContext('2d');canvas.width=size;canvas.height=size;const scale=Math.max(size/image.naturalWidth,size/image.naturalHeight),width=image.naturalWidth*scale,height=image.naturalHeight*scale;context.drawImage(image,(size-width)/2,(size-height)/2,width,height);DKD.state.profile.photo=canvas.toDataURL('image/jpeg',.84);DKD.persist();if(DKD.route==='settings')DKD.renderSettings();else DKD.render();DKD.toast('Profile photo updated','Your new profile photo is now shown across DraBornPS.');resolve();};image.src=reader.result;};reader.readAsDataURL(file);});
 const actions={
 ...
 'reset-console-confirm':()=>DKD.modal('DraBornPS Sıfırla?',`<p>DraBornPS üzerindeki yerel profil, profil resmi, skorlar, kupalar, ayarlar, satın alma geçmişi, indirmeler ve kişiselleştirmeler varsayılan duruma dönecek.</p><p class="small muted">Bu işlem yalnızca bu tarayıcıdaki DraBornPS verilerini sıfırlar.</p><div class="modal-actions"><button class="pill" data-action="close-modal">İptal</button><button class="pill primary" data-action="reset-console">Sıfırla</button></div>`),
 'reset-console':()=>{try{localStorage.removeItem('dkd_ps5_state');}catch{}location.reload();},
