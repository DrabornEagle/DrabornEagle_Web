-- DraBornDeal / Firsat Radari v0.4
-- Product snapshot, price history, price-drop signal and score refresh layer.
...
select
  ...
  case
    when lag(dkd_snapshot.dkd_current_price) over (partition by dkd_snapshot.dkd_product_id order by dkd_snapshot.dkd_scraped_at) is null then null
    else round(dkd_snapshot.dkd_current_price - lag(dkd_snapshot.dkd_current_price) over (partition by dkd_snapshot.dkd_product_id order by dkd_snapshot.dkd_scraped_at), 2)
  end as dkd_price_change_amount,
  case
    when lag(dkd_snapshot.dkd_current_price) over (partition by dkd_snapshot.dkd_product_id order by dkd_snapshot.dkd_scraped_at) is null then null
    when lag(dkd_snapshot.dkd_current_price) over (partition by dkd_snapshot.dkd_product_id order by dkd_snapshot.dkd_scraped_at) = 0 then null
    else round(((dkd_snapshot.dkd_current_price - lag(dkd_snapshot.dkd_current_price) over (partition by dkd_snapshot.dkd_product_id order by dkd_snapshot.dkd_scraped_at)) / lag(dkd_snapshot.dkd_current_price) over (partition by dkd_snapshot.dkd_product_id order by dkd_snapshot.dkd_scraped_at)) * 100, 2)
...
with dkd_history as (
  select * from public.dkd_deal_product_price_history_v0_4
)
select
...
insert into public.dkd_deal_system_settings (dkd_setting_key, dkd_setting_value, dkd_description) values
  ('dkd_deal_price_history_version', '{"version":"0.4","views":["dkd_deal_product_price_history_v0_4","dkd_deal_product_metrics_v0_4","dkd_deal_price_drop_alerts_v0_4"],"price_drop_threshold_percent":3}'::jsonb, 'DraBornDeal price history and price drop layer.'),
