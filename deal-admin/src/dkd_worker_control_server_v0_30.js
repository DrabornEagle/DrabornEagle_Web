...
const dkdWorkerFile = 'src/dkd_campaign_worker_v0_30.js';
const dkdVersion = 'v0.30';
let dkdProc = null;
...
dkdApp.post('/api/dkd-worker-start', dkdAuth, (req, res) => {
  ...
  dkdAddLog(`worker_started version=${dkdVersion} file=${dkdWorkerFile} pid=${dkdProc.pid}`);
  dkdProc.stdout.on('data', (dkdChunk) => dkdAddLog(dkdChunk.toString()));
  dkdProc.stderr.on('data', (dkdChunk) => dkdAddLog(`ERR ${dkdChunk.toString()}`));
  dkdProc.on('exit', (dkdCode, dkdSignal) => { dkdStoppedAt = new Date().toISOString(); dkdLastExit = { dkd_code: dkdCode, dkd_signal: dkdSignal, dkd_time: dkdStoppedAt }; dkdAddLog(`worker_exited code=${dkdCode} signal=${dkdSignal || ''}`); dkdProc = null; });
  res.json({ dkd_ok: true, dkd_started: true, dkd_state: dkdState() });
});
dkdApp.post('/api/dkd-worker-stop', dkdAuth, (req, res) => { if (!dkdProc || dkdProc.killed) return res.json({ dkd_ok: true, dkd_already_stopped: true, dkd_state: dkdState() }); const dkdPid = dkdProc.pid; dkdProc.kill('SIGTERM'); dkdAddLog(`worker_stop_requested pid=${dkdPid}`); res.json({ dkd_ok: true, dkd_stopping: true, dkd_state: dkdState() }); });
