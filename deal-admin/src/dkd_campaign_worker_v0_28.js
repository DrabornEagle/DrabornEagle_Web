...
const dkdAttemptLimit = Math.max(5, Math.min(Number(process.env.DKD_WORKER_ATTEMPT_LIMIT || 20), 60));
let dkdRunning = false;

function dkdLog(dkdPayload) { console.log(JSON.stringify({ dkd_worker: 'drabornbee_campaign_v0_28', dkd_time: new Date().toISOString(), ...dkdPayload })); }
...
async function dkdRunOnce() {
  ...
  let dkdAttempts = 0;
  try {
    dkdLog({ dkd_message: 'campaign_started', dkd_share_limit: dkdShareLimit, dkd_attempt_limit: dkdAttemptLimit });
    const dkdTrendyol = await dkdDiscoverTrendyolApiUrlsV026();
    const dkdCampaign = await dkdDiscoverCampaignUrlsV028();
    const dkdUrls = dkdMerge(dkdTrendyol.dkd_product_urls, dkdCampaign.dkd_product_urls);
    dkdLog({ dkd_message: 'campaign_discovery_done', dkd_trendyol_count: dkdTrendyol.dkd_product_url_count || 0, dkd_campaign_count: dkdCampaign.dkd_product_url_count || 0, dkd_total_count: dkdUrls.length, dkd_trendyol_logs: (dkdTrendyol.dkd_logs || []).slice(0, 8), dkd_campaign_logs: (dkdCampaign.dkd_logs || []).slice(0, 8) });
    for (const dkdUrl of dkdUrls) {
      ...
      try {
        ...
        dkdLog({ dkd_message: 'try_campaign_share', dkd_attempts: dkdAttempts, dkd_url: dkdUrl });
        const dkdResult = await dkdAdminDirectShareFullUrlOnly(dkdSupabase, dkdBotToken, dkdUrl);
        dkdShared += 1;
        dkdLog({ dkd_message: 'campaign_shared', dkd_url: dkdUrl, dkd_result: dkdResult });
        await dkdSleep(900);
      } catch (dkdError) {
        dkdSkipped += 1;
        dkdLog({ dkd_message: 'campaign_skipped', dkd_url: dkdUrl, dkd_error: dkdText(dkdError) });
      }
    }
    dkdLog({ dkd_message: 'campaign_finished', dkd_shared: dkdShared, dkd_skipped: dkdSkipped, dkd_attempts: dkdAttempts });
  } catch (dkdError) {
    dkdLog({ dkd_message: 'campaign_failed', dkd_error: dkdText(dkdError) });
  } finally {
    dkdRunning = false;
  }
}

dkdLog({ dkd_message: 'campaign_worker_started', dkd_interval_minutes: dkdIntervalMinutes, dkd_share_limit: dkdShareLimit, dkd_attempt_limit: dkdAttemptLimit });
await dkdRunOnce();
