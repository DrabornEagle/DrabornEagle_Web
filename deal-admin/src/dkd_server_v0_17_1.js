...
function dkdErrorText(error) {
  ...
  if (typeof error === 'string') return error || 'Bilinmeyen hata';
  if (error instanceof Error && error.message) return error.message;
  if (error.message) return String(error.message);
  try {
    const dkdJson = JSON.stringify(error);
...
async function dkdCountRows(dkdTable, dkdBuildQuery = null) {
  let dkdQuery = dkdSupabase.from(dkdTable).select('*', { count: 'exact', head: true });
  if (dkdBuildQuery) dkdQuery = dkdBuildQuery(dkdQuery);
  const { count, error } = await dkdQuery;
  if (error) throw error;
  return Number(count || 0);
}
async function dkdSafeValue(dkdWarnings, dkdLabel, dkdFallback, dkdTask) {
  try {
    return await dkdTask();
  } catch (error) {
    dkdWarnings.push({ dkd_label: dkdLabel, dkd_error: dkdErrorText(error) });
    return dkdFallback;
  }
}
async function dkdSafeQueryRows(dkdWarnings, dkdLabel, dkdQuery, dkdFallback = []) {
  return dkdSafeValue(dkdWarnings, dkdLabel, dkdFallback, async () => {
    const { data, error } = await dkdQuery;
    if (error) throw error;
    return data || dkdFallback;
  });
}
...
async function dkdBuildStats() {
  ...
  const [dkdTotalProducts, dkdTodayProducts, dkdPublishedPosts, dkdTodayPosts, dkdPendingWatchLinks, dkdSourcesRows, dkdProductSourceRows, dkdTopHotRows] = await Promise.all([
    dkdSafeValue(dkdWarnings, 'total_products', 0, () => dkdCountRows('dkd_deal_products')),
    dkdSafeValue(dkdWarnings, 'today_products', 0, () => dkdCountRows('dkd_deal_products', (dkdQuery) => dkdQuery.gte('dkd_updated_at', dkdTodayIso))),
    dkdSafeValue(dkdWarnings, 'published_telegram_posts', 0, () => dkdCountRows('dkd_deal_social_posts', (dkdQuery) => dkdQuery.eq('dkd_platform', 'telegram').eq('dkd_status', 'published'))),
    dkdSafeValue(dkdWarnings, 'today_telegram_posts', 0, () => dkdCountRows('dkd_deal_social_posts', (dkdQuery) => dkdQuery.eq('dkd_platform', 'telegram').gte('dkd_published_at', dkdTodayIso))),
    dkdSafeValue(dkdWarnings, 'pending_watch_links', 0, () => dkdCountRows('dkd_deal_watch_link_queue_v0_7_2', (dkdQuery) => dkdQuery.in('dkd_status', ['pending', 'accepted']))),
    dkdSafeQueryRows(dkdWarnings, 'sources', dkdSupabase.from('dkd_deal_sources').select('dkd_id, dkd_source_key, dkd_source_name, dkd_is_active').eq('dkd_country_code', 'TR').order('dkd_priority'), []),
    dkdSafeQueryRows(dkdWarnings, 'product_source_rows', dkdSupabase.from('dkd_deal_products').select('dkd_source_id').limit(1000), []),
    dkdSafeQueryRows(dkdWarnings, 'top_hot_products', dkdSupabase.from('dkd_deal_hot_ranked_products_v0_5').select('dkd_product_name, dkd_source_key, dkd_current_price, dkd_currency_code, dkd_hot_score, dkd_heat_label, dkd_discount_percent').order('dkd_hot_score', { ascending: false }).limit(5), [])
  ]);

  const dkdSourceMap = new Map((dkdSourcesRows || []).map((dkdSource) => [dkdSource.dkd_id, dkdSource]));
  const dkdSourceCounts = new Map();
  (dkdProductSourceRows || []).forEach((dkdRow) => {
  ...
  return {
    dkd_stats_version: 'v0.17.1',
    dkd_generated_at: new Date().toISOString(),
    dkd_warnings: dkdWarnings,
    dkd_totals: {
      ...
      dkd_active_sources: (dkdSourcesRows || []).filter((dkdSource) => dkdSource.dkd_is_active).length
    },
    dkd_sources: dkdSources,
    dkd_top_hot_products: dkdTopHotRows || []
  };
}
...
dkdApp.get('/api/dkd-stats', dkdRequireAdmin, async (req, res) => {
  try {
    ...
    res.status(200).json({ dkd_ok: true, dkd_stats: { dkd_stats_version: 'v0.17.1', dkd_warnings: [{ dkd_label: 'stats', dkd_error: dkdErrorText(error) }], dkd_totals: {}, dkd_sources: [], dkd_top_hot_products: [] } });
  }
});
...
dkdApp.listen(dkdPort, '0.0.0.0', () => console.log(JSON.stringify({ dkd_message: 'dkd_admin_started', dkd_port: dkdPort, dkd_direct_share: true, dkd_full_url_only: true, dkd_telegram_preview: true, dkd_cleanup_actions: true, dkd_stats: true, dkd_version: 'v0.17.1' })));
