...
function dkdLog(dkdPayload) { console.log(JSON.stringify({ dkd_worker: 'drabornbee_campaign_v0_30', dkd_time: new Date().toISOString(), ...dkdPayload })); }
...
async function dkdRunOnce() {
  ...
  let dkdAttempts = 0;
  try {
    dkdLog({ dkd_message: 'v030_started', dkd_share_limit: dkdShareLimit, dkd_attempt_limit: dkdAttemptLimit });
    ...
    const dkdUrls = dkdMerge(dkdApi.dkd_product_urls, dkdRender.dkd_product_urls, dkdCampaign.dkd_product_urls);
    dkdLog({ dkd_message: 'v030_discovery_done', dkd_trendyol_api_count: dkdApi.dkd_product_url_count || 0, dkd_trendyol_render_count: dkdRender.dkd_product_url_count || 0, dkd_campaign_count: dkdCampaign.dkd_product_url_count || 0, dkd_total_count: dkdUrls.length, dkd_render_logs: (dkdRender.dkd_logs || []).slice(0, 6), dkd_api_logs: (dkdApi.dkd_logs || []).slice(0, 6) });
    ...
    dkdLog({ dkd_message: 'v030_finished', dkd_shared: dkdShared, dkd_skipped: dkdSkipped, dkd_attempts: dkdAttempts });
  } catch (dkdError) {
    dkdLog({ dkd_message: 'v030_failed', dkd_error: dkdText(dkdError) });
  } finally {
    dkdRunning = false;
  }
}

dkdLog({ dkd_message: 'v030_worker_started', dkd_interval_minutes: dkdIntervalMinutes, dkd_share_limit: dkdShareLimit, dkd_attempt_limit: dkdAttemptLimit });
await dkdRunOnce();
