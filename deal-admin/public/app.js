...
function dkdHealthView(dkdRow){
  ...
  if(dkdKey === 'dkd_deal_affiliate_links_version') return {dkdTitle:'Affiliate sistemi',dkdDescription:`Kaynak bazlı affiliate kuralları hazır. Varsayılan: gerçek bilgi girilene kadar kapalı.`,dkdPills:['v0.9','hazır']};
...
function dkdRenderAffiliateRules(dkdRows){dkdSetHtml('dkdAffiliateRules',dkdRows.map((r)=>`<div class="dkd-item"><div class="dkd-title">${dkdEscape(r.dkd_source_name)} — ${r.dkd_is_active?'aktif':'kapalı'}</div><div class="dkd-meta">Yöntem: ${dkdEscape(dkdTrMode(r.dkd_affiliate_mode))} · Eklenen bilgi: ${dkdEscape(r.dkd_param_key||'-')}=${dkdEscape(r.dkd_param_value||'-')}</div>${dkdPill(r.dkd_source_key,r.dkd_is_active?'hot':'warn')}<div class="dkd-meta">Nasıl çalışır: Ürün bağlantısı Telegram’a giderken, aşağıdaki bilgi bağlantının sonuna eklenir. Örnek: ?ref=seninKodun veya mevcut bağlantıda &ref=seninKodun.</div><form class="dkd-affiliate-form" data-rule-id="${dkdEscape(r.dkd_affiliate_rule_id)}"><select name="dkd_affiliate_mode"><option value="append_query" ${r.dkd_affiliate_mode==='append_query'?'selected':''}>Bağlantının sonuna ekle</option><option value="template" ${r.dkd_affiliate_mode==='template'?'selected':''}>Hazır bağlantı şablonu</option><option value="none" ${r.dkd_affiliate_mode==='none'?'selected':''}>Kullanma</option></select><input name="dkd_param_key" placeholder="Ek bilgi adı örn: ref, aff, tag" value="${dkdEscape(r.dkd_param_key||'')}"><input name="dkd_param_value" placeholder="Affiliate kodun / mağaza takip kodun" value="${dkdEscape(r.dkd_param_value||'')}"><button type="submit">Kaydet</button><label class="dkd-switch"><input type="checkbox" name="dkd_is_active" ${r.dkd_is_active?'checked':''}> Aktif</label><input name="dkd_template_url" placeholder="Hazır bağlantı şablonu, {url} ürün bağlantısı yerine geçer" value="${dkdEscape(r.dkd_template_url||'')}"></form></div>`).join(''));document.querySelectorAll('.dkd-affiliate-form').forEach((form)=>form.addEventListener('submit',dkdSaveAffiliateRule));}
