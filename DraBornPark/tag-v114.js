/* DraBornPark public tag runtime v1.0.14 — visible version refresh and focused copy cleanup. */
(()=>{
  ...
  const refreshVersion=()=>{
    ...
    if(live){
      ...
      live.dataset.dkdVersion='114';
    }
    document.querySelectorAll('*').forEach(el=>{
      if(el.children.length)return;
      const text=(el.textContent||'').trim();
      if(/^v1\.0\.(?:[0-9]|1[0-3])$/i.test(text))el.textContent='v1.0.14';
      if(/^DraBornPark v1\.0\.(?:[0-9]|1[0-3]) güvenli araç ağına bağlanılıyor/i.test(text))el.textContent='DraBornPark v1.0.14 güvenli araç ağına bağlanılıyor…';
    });
  };
