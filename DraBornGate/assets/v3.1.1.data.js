const DKD_V311_VERSION = '3.1.1';
const DKD_V311_PROJECT_REF = 'guuwomvszlwhkmstewfl';
const DKD_V311_API_URL = 'https://guuwomvszlwhkmstewfl.supabase.co';
const DKD_V311_API_KEY = 'sb_publishable_bf1URxrlLlvMQ8e1Z7oxkQ_jx9mvy5g';
const DKD_V311_AUTH_STORAGE_KEY = `sb-${DKD_V311_PROJECT_REF}-auth-token`;
...
function dkdV31ReadableError(dkdValue, dkdDepth = 0) {
  if (dkdDepth > 6 || dkdValue == null) return '';
  ...
  if (typeof dkdValue === 'string') {
    ...
    try {
      const dkdParsed = JSON.parse(dkdText);
      return dkdV31ReadableError(dkdParsed, dkdDepth + 1) || dkdText;
    } catch {
      return dkdText;
    }
  }
  if (Array.isArray(dkdValue)) {
    for (const dkdItem of dkdValue) {
      const dkdMessage = dkdV31ReadableError(dkdItem, dkdDepth + 1);
      if (dkdMessage) return dkdMessage;
    }
    return '';
  }
  if (typeof dkdValue === 'object') {
    for (const dkdKey of ['message', 'error_description', 'error', 'details', 'hint', 'description', 'msg']) {
      const dkdMessage = dkdV31ReadableError(dkdValue[dkdKey], dkdDepth + 1);
      if (dkdMessage) return dkdMessage;
    }
  }
  return '';
...
function dkdV31IsProjectAccessToken(dkdToken, dkdAllowExpired = false) {
  ...
  const dkdPayload = dkdV31DecodeJwtPayload(dkdToken);
  ...
  if (!dkdIssuer.includes(DKD_V311_PROJECT_REF) && dkdRef !== DKD_V311_PROJECT_REF) return false;
...
function dkdV31FindSessionInValue(dkdValue, dkdDepth = 0) {
  ...
  if (typeof dkdValue === 'string') {
    if (dkdV31IsProjectAccessToken(dkdValue, true)) return { access_token: dkdValue };
    try {
      return dkdV31FindSessionInValue(JSON.parse(dkdValue), dkdDepth + 1);
    } catch {
      return null;
    }
  }
  if (Array.isArray(dkdValue)) {
    for (const dkdItem of dkdValue) {
      const dkdSession = dkdV31FindSessionInValue(dkdItem, dkdDepth + 1);
      if (dkdSession) return dkdSession;
    }
    return null;
  }
  if (typeof dkdValue === 'object') {
    if (dkdV31IsProjectAccessToken(dkdValue.access_token, true)) {
      return {
    ...
    for (const dkdKey of ['currentSession', 'session', 'data', 'value']) {
      const dkdSession = dkdV31FindSessionInValue(dkdValue[dkdKey], dkdDepth + 1);
      if (dkdSession) return dkdSession;
    }
  }
  return null;
...
function dkdV31ReadStoredSession() {
  const dkdKeys = [DKD_V311_AUTH_STORAGE_KEY];
  for (const dkdStore of [localStorage, sessionStorage]) {
    for (let dkdIndex = 0; dkdIndex < dkdStore.length; dkdIndex += 1) {
      ...
      if (dkdKey.includes(DKD_V311_PROJECT_REF) && /auth-token|session/i.test(dkdKey) && !dkdKeys.includes(dkdKey)) dkdKeys.push(dkdKey);
    }
...
async function dkdV31RefreshStoredSession(dkdSession) {
  ...
  const dkdResponse = await fetch(`${DKD_V311_API_URL}/auth/v1/token?grant_type=refresh_token`, {
    method: 'POST',
    cache: 'no-store',
    headers: {
      apikey: DKD_V311_API_KEY,
  ...
  try {
    (dkdSession.storage || localStorage).setItem(dkdSession.storageKey || DKD_V311_AUTH_STORAGE_KEY, JSON.stringify(dkdPayload));
  } catch {
    // Oturum belleğe yazılamasa da yenilenen anahtar bu çağrıda kullanılabilir.
  }
  return dkdPayload.access_token;
}

async function dkdV31ResolveAccessToken(dkdForceRefresh = false) {
  const dkdSession = dkdV31ReadStoredSession();
  if (!dkdSession) return '';
  if (!dkdForceRefresh && dkdV31IsProjectAccessToken(dkdSession.access_token)) return dkdSession.access_token;
  return (await dkdV31RefreshStoredSession(dkdSession)) || '';
}

async function dkdV31WaitForAccessToken(dkdTimeoutMs = 9000, dkdForceRefresh = false) {
  const dkdStartedAt = Date.now();
  while (Date.now() - dkdStartedAt < dkdTimeoutMs) {
    const dkdToken = await dkdV31ResolveAccessToken(dkdForceRefresh);
...
async function dkdV31RpcRequest(dkdFunctionName, dkdArguments, dkdToken) {
  const dkdResponse = await fetch(`${DKD_V311_API_URL}/rest/v1/rpc/${encodeURIComponent(dkdFunctionName)}`, {
    method: 'POST',
    cache: 'no-store',
    headers: {
      apikey: DKD_V311_API_KEY,
...
async function dkdV31Rpc(dkdFunctionName, dkdArguments = {}) {
  ...
  if (dkdResult.response.status === 401) {
    dkdToken = await dkdV31WaitForAccessToken(5000, true);
  ...
  if (!dkdResult.response.ok) {
    ...
    if (dkdResult.response.status === 401 || /suitable key|wrong key|jwt|token/i.test(dkdMessage)) {
      dkdMessage = 'DraBornGate oturum anahtarı geçersiz veya süresi dolmuş. Çıkış yapıp tekrar giriş yapın.';
    }
...
async function dkdV31LoadQueue() {
  ...
  try {
    ...
    dkdV31State.queueError = dkdV31ReadableError(dkdError) || 'Canlı kurye kuyruğu alınamadı.';
    throw dkdError;
  } finally {
...
async function dkdV31LoadAdminCatalog() {
  ...
  try {
    ...
    dkdV31State.adminLoading = false;
  }
}
...
window.dkdV31Data = {
  version: DKD_V311_VERSION,
  state: dkdV31State,
  normalize: dkdV31Normalize,
  escape: dkdV31Escape,
  readableError: dkdV31ReadableError,
