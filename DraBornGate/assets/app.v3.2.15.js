const DKD_V3215_BOOT_VERSION = '3.2.15';
const DKD_V3215_BOOT_REVISION = 'earnings-moto1';

function dkdV3215BootFailure(dkdError) {
  console.error('DraBornGate v3.2.15 açılış hatası:', dkdError);
...
function dkdV3215ResetStaleDom() {
  ...
  if (dkdRoot) {
    dkdRoot.innerHTML = '<div class="boot-shell"><div class="boot-logo"><span>DBG</span></div><div class="boot-copy"><strong>DraBornGate Web v3.2.15</strong><span>Kurye menüsü ve motosiklet ikonları hazırlanıyor</span></div><div class="boot-progress"><i></i></div></div>';
  }
}

async function dkdV3215PrepareFreshRuntime() {
  dkdV3215ResetStaleDom();
  sessionStorage.setItem('dkd_gate_web_version', DKD_V3215_BOOT_VERSION);
  localStorage.setItem('dkd_gate_last_web_release', `${DKD_V3215_BOOT_VERSION}-${DKD_V3215_BOOT_REVISION}`);
  document.documentElement.dataset.dkdGateVersion = DKD_V3215_BOOT_VERSION;
...
async function dkdV3215BootWait(dkdTimeout = 30000) {
  ...
  throw new Error('DraBornGate v3.2.15 veri katmanı zamanında hazırlanamadı.');
}

async function dkdV3215BootStyle(dkdPath, dkdDatasetKey) {
  ...
  await new Promise((dkdResolve, dkdReject) => {
    ...
    dkdLink.href = `${dkdPath}?v=${DKD_V3215_BOOT_VERSION}-${DKD_V3215_BOOT_REVISION}`;
...
function dkdV3215PartnerDataBridge(dkdOriginalData) {
...
async function dkdV3215RegisterWorker() {
  if (!('serviceWorker' in navigator)) return;
  const dkdRegistration = await navigator.serviceWorker.register(
    `./sw.js?v=${DKD_V3215_BOOT_VERSION}-${DKD_V3215_BOOT_REVISION}`,
...
async function dkdV3215Start() {
  await import(`./v3.2.15.guard.js?v=${DKD_V3215_BOOT_VERSION}-${DKD_V3215_BOOT_REVISION}`);
  await dkdV3215PrepareFreshRuntime();
  await import(`./app.js?v=${DKD_V3215_BOOT_VERSION}-${DKD_V3215_BOOT_REVISION}-core`);
  await dkdV3215BootWait();

  if (!dkdV3215BootIsSimple()) {
    await dkdV3215BootStyle('./assets/v3.2.11.css', 'dkd-web-v3211');
    await dkdV3215BootStyle('./assets/v3.2.15.css', 'dkd-web-v3215');
    const dkdOriginalData = window.dkdV31Data;
    window.dkdV31Data = dkdV3215PartnerDataBridge(dkdOriginalData);
    try {
      await import(`./v3.2.11.js?v=${DKD_V3215_BOOT_VERSION}-${DKD_V3215_BOOT_REVISION}-earnings`);
      await import(`./v3.2.15.js?v=${DKD_V3215_BOOT_VERSION}-${DKD_V3215_BOOT_REVISION}-ui`);
    } finally {
  ...
  document.documentElement.dataset.dkdGateVersion = DKD_V3215_BOOT_VERSION;
  sessionStorage.setItem('dkd_gate_web_version', DKD_V3215_BOOT_VERSION);
  window.__DKD_GATE_WEB_VERSION__ = DKD_V3215_BOOT_VERSION;
  window.__DKD_GATE_V3215_ACTIVE__ = true;
  await dkdV3215RegisterWorker();
  window.dkdV3215Cleanup?.();
}

dkdV3215Start().catch(dkdV3215BootFailure);
