(() => {
  ...
  const DKD_V329_PATTERN = /(?:DraBornGate\s+Web\s+)?v(?:2(?:\.\d+){0,2}|3\.(?:0|1)(?:\.\d+)?|3\.2(?:\.[0-8])?)(?!\.\d)\b/gi;

  function dkdV329ReplaceVersion(dkdValue) {
    const dkdText = String(dkdValue || '');
    DKD_V329_PATTERN.lastIndex = 0;
    if (!DKD_V329_PATTERN.test(dkdText)) return dkdText;
    DKD_V329_PATTERN.lastIndex = 0;
    return dkdText.replace(DKD_V329_PATTERN, (dkdMatch) =>
  ...
  function dkdV329PatchMetadata() {
    document.title = dkdV329ReplaceVersion(document.title);
    for (const dkdMeta of document.querySelectorAll('meta[name="description"],meta[name="application-name"]')) {
      ...
      dkdMeta.setAttribute('content', dkdV329ReplaceVersion(dkdCurrent));
    }
  }
